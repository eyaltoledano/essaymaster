...
agent skill (a procedure document plus eight reference files encoding the rules
above), together with scaffold templates, miner and reviewer subagents, and the
drift hook, for the Claude Code harness~\citep{claudecode}. We view this packaging
...
must be made \emph{visible} (a drift count printed at session start) and falsehood
...
in one line at session start. A sync is then a bounded editorial procedure: classify
...
among six modes (mine, init, sync, build, review, publish); eight reference files
carry the rules of \S\ref{sec:design} in the imperative, checklist-bearing form
agents follow reliably. The mechanical parts are plain scripts: drift counting,
...
Two subagents matter. The \emph{miner} performs read-only archaeology: given a
repository slice, it extracts candidate paper material (optimization campaigns
with recorded numbers, non-obvious theses in design docs, negative results), each
find paired with a web-verified novelty check. The \emph{reviewer} is adversarial
